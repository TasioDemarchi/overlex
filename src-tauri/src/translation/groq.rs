...
/// Groq Llama 3.1 8B Instant translation adapter
...
#[async_trait]
impl TranslationEngine for GroqAdapter {
    async fn translate(
        &self,
        text: &str,
        source: &str,
        ...
        if status.as_u16() == 401 || status.as_u16() == 403 {
            ...
            app_log!("[GROQ] Auth error HTTP {}: {}", status.as_u16(), &body_text[..body_text.len().min(200)]);
        ...
        if !status.is_success() {
            ...
            app_log!("[GROQ] API error HTTP {}: {}", status_code, &body_text[..body_text.len().min(200)]);
...
mod tests {
    ...
    fn test_system_instruction_with_full_context() {
        ...
        let instruction = GroqAdapter::build_system_instruction("en", "es", Some(&ctx));
    ...
    fn test_system_instruction_with_process_only() {
        ...
        let instruction = GroqAdapter::build_system_instruction("en", "ja", Some(&ctx));
    ...
    fn test_system_instruction_without_context() {
        let instruction = GroqAdapter::build_system_instruction("auto", "es", None);
